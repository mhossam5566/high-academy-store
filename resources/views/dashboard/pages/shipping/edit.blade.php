...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">
                <span class="text-muted fw-light">طرق الشحن /</span> تعديل طريقة الشحن #{{ $shippingMethod->id }}
            </h4>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('dashboard.shipping-methods.update', $shippingMethod->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-4">
                                ...
                                <select id="type" name="type"
                                    ...
                                    <option value="post"
                                        {{ old('type', $shippingMethod->type) == 'post' ? 'selected' : '' }}>مكتب بريد
                                    </option>
                                    <option value="home"
                                        {{ old('type', $shippingMethod->type) == 'home' ? 'selected' : '' }}>توصيل لباب
                                        البيت</option>
                                    <option value="branch"
                                        {{ old('type', $shippingMethod->type) == 'branch' ? 'selected' : '' }}>استلام من
                                        المكتبة</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" id="fee" name="fee" step="0.01"
                                    ...
                                    value="{{ old('fee', $shippingMethod->fee ?? '0.00') }}" placeholder="0.00">
                                @error('fee')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
