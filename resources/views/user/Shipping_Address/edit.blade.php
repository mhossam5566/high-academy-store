...
@section('content')
                        ...
                        <div class="row gy-2 mt-2">
                            ...
                            <div class="form-mobile">
                                ...
                                <input class="form-control" type="number" id="mobile" name="mobile" pattern="\d{11}"
                                    value="{{ $address->mobile }}" minlength="11" maxlength="11"
                            ...
                            <div class="form-mobile">
                                ...
                                <input class="form-control" type="number" id="temp_mobile" name="temp_mobile"
                                    value="{{ $address->temp_mobile }}" pattern="\d{11}" minlength="11" maxlength="11"
                                    placeholder="رقم موبايل الاحتياطي" required />
                            </div>

                            <div class="form-group">
                                <label for="near_post">اسم اقرب مكتب بريد</label>
                                <input class="form-control" id="near_post" name="near_post"
                                    value="{{ $address->near_post }}" placeholder="اسم اقرب مكتب بريد" />
                            </div>
                        </div>
                </div>
            </div>
            <div class="my-5 text-center d-flex justify-content-center">
                <button class="btn bg-dropdown-menu profile-button text-white rounded-2 submit-btn" type="submit">Save
                </button>
                <a user_address_id="{{ $address->id }}" class="delete_btn delete-button ms-2">حذف البيانات</a>
            </div>
            </form>
    ...
    <script>
        $(document).ready(function() {
            document.getElementById('governorates').value = "{{ $address->governorate }}";
            document.getElementById('governorates').dispatchEvent(new Event('change'));
            document.getElementById('cities').value = "{{ $address->city }}";
            ...
            $('#form').submit(function(e) {
                ...
                $.ajax({
                    url: '{{ route('user.shipping.update') }}',
                    type: "POST",
                    dataType: "json",
                    data: formData,
                    contentType: false,
