<style>
  ...
  .nav-search button:hover { background: var(--nav-accent-light); }
  ...
  .nav-search-mobile input {
    ...
    padding: 9px 14px; font-family: 'Cairo', sans-serif;
    font-size: 13px; color: #fff; direction: rtl;
  }
  .nav-search-mobile input::placeholder { color: var(--nav-muted); }
  .nav-search-mobile button {
    background: var(--nav-accent); border: none; padding: 9px 16px;
  ...
  .sidebar-profile:hover { background: var(--nav-surface); }
  .sidebar-avatar {
  ...
  .sidebar-nav-link:hover .nav-icon  { background: rgba(255,255,255,.12); }
  .sidebar-nav-link.active .nav-icon { background: rgba(224,123,57,.25); }

  /* Dropdown inside sidebar */
  .sidebar-dropdown { display: none; padding: 4px 0 4px 14px; }
  ...
  .sidebar-dropdown-link:hover { background: var(--nav-surface); color: #fff; }

  /* Logout / Danger */
  .sidebar-nav-link.danger { color: #fc8181; }
  .sidebar-nav-link.danger:hover { background: rgba(220,38,38,.15); color: #fc8181; }
  .sidebar-nav-link.danger .nav-icon { background: rgba(220,38,38,.15); }

  /* ═══ Responsive ═══ */
  @media (max-width: 600px) {
    .nav-logo .logo-high,
    .nav-logo .logo-academy { font-size: 12px; padding: 3px 7px; }
    .nav-search { display: none; }
    .nav-search-mobile { display: block; }
    .main-navbar { padding: 0 12px; }
    .app-sidebar { width: 270px; right: -270px; }
  }
...
<div class="app-sidebar" id="sidebar">
  ...
  <a href="{{ route('user.myaccount') }}" class="sidebar-profile">
    <img
      src="{{ Auth()->user()->profile_image ? asset('storage/images/user/' . Auth()->user()->profile_image) : asset('storage/images/pngegg.png') }}"
  ...
  <ul class="sidebar-nav">
    ...
    <li>
      <a href="{{ route('user.home') }}" class="sidebar-nav-link {{ $lastSegment == '' || $lastSegment == 'ar' ? '' : '' }}">
    ...
    <li>
      <a href="{{ route('user.orders.user') }}" class="sidebar-nav-link {{ $lastSegment == 'myorders' ? 'active' : '' }}">
        <span class="nav-icon">📦</span>
        طلباتي
      </a>
    </li>
    ...
    <li>
      <a href="{{ route('user.vochers.user') }}" class="sidebar-nav-link {{ $lastSegment == 'myvouchers' ? 'active' : '' }}">
        <span class="nav-icon">🎁</span>
        أكوادي
      </a>
    </li>
    ...
    <li>
      <a href="{{ route('user.shipping') }}" class="sidebar-nav-link {{ $lastSegment == 'shipping' ? 'active' : '' }}">
        <span class="nav-icon">🚚</span>
        عناوين استلام شحنتك
      </a>
    </li>
    ...
    <li>
      <button class="sidebar-nav-link" onclick="toggleContactDropdown()" id="contactToggle">
        ...
        تواصل معنا
        <i class="fas fa-chevron-left" id="contactChevron" style="margin-right:auto; margin-left:0; font-size:11px; transition:transform .2s; color:var(--nav-muted);"></i>
      </button>
      <div class="sidebar-dropdown" id="contactDropdown">
        ...
        <a href="https://www.whatsapp.com/channel/0029VbAlwWH8fewxAkAdCZ23" target="_blank" class="sidebar-dropdown-link">
          <span style="color:#25d366; font-size:15px;"><i class="fab fa-whatsapp"></i></span>
          قناة واتساب
        </a>
      </div>
...
<script>
  ...
  function closeSidebarFn(){ sidebar.classList.remove('show'); overlay.classList.remove('show'); document.body.style.overflow=''; }

  sidebarToggle.addEventListener('click', e => { sidebar.classList.contains('show') ? closeSidebarFn() : openSidebar(); e.stopPropagation(); });
